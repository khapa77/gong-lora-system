// LoRa Ra-02 Simple Test - CLIENT
// Listens for ping from server, sends pong response
...
void setup() {
  ...
  radio.startReceive();
  Serial.println("[CLIENT] Listening for PING... will reply with PONG\n");
}
    ...
    if (len > 0 && len <= 64) {
      ...
      if (radio.readData((uint8_t*)buf, len) == RADIOLIB_ERR_NONE) {
        ...
        if (strncmp(buf, "PING", 4) == 0) {
          char reply[32];
          snprintf(reply, sizeof(reply), "PONG for %s", buf);

          delay(50); // Small delay before reply

          dioFlag = false;
          int state = radio.startTransmit(reply);
          if (state == RADIOLIB_ERR_NONE) {
            txBusy = true;
            Serial.printf("[TX] %s\n", reply);
          } else {
