...
//   - Окно ACK (ACK_SLOT_COUNT слотов) стало ~8 * ~12с ≈ 1.6 минуты вместо ~2с.
//     Реальный период heartbeat = airtime heartbeat + окно ACK (~2 минуты):
//     сервер ставит heartbeat в очередь только когда окно закрыто и эфир
//     свободен (иначе поле "time" в нём устаревало на всё окно).
//   - GONG/STOP идут через отдельную приоритетную очередь и не ждут окна.
...
// ACK_SLOT_COUNT = 8 при парке из 4 клиентов (см. BOM): окно вдвое короче,
// чем при 16. Слот клиента меняется с каждым heartbeat (хеш id+seq, см.
// client ackSlot()), поэтому совпадение слотов двух клиентов — случайное
// событие одного цикла, а не постоянная "невидимость" пары клиентов.
