...
// Реальный парк — 4 клиента (см. BOM), не 16 — ACK_SLOT_COUNT уменьшен до 8
// (см. ниже), окно ACK и период обновления статуса клиентов в вебе сократились
// вдвое (~96с/~1.6мин вместо ~192с/~3.2мин), clientTimeout — втрое (~4.8мин
// вместо ~9.6мин).
//
// Слот = не гарантия отсутствия коллизий, а её вероятность. ACK_SLOT_COUNT=8
// с 4 клиентами (FNV-1a(id) % 8, см. ackSlot() в lorahandler.cpp) даёт ~59%
// шанс, что у КАКОЙ-ТО пары из 4 клиентов слот совпадёт — а раз слот
// вычисляется из MAC-производного ID, совпавшая пара будет коллизировать
// НАВСЕГДА, на каждом heartbeat, а не время от времени. Последствие
// не критично: GONG/STOP не ждут ACK и приходят всем независимо; ACK — это
// только диагностика (RSSI/SNR/resp_ms в /api/state), так что коллизия
// проявится как "клиент периодически выпадает из списка в вебе, хотя гонг
// у него исправно играет". Если это случится — либо поднять ACK_SLOT_COUNT
// обратно (16 даёт ~33% шанс, всё ещё не ноль), либо задать одному из
// столкнувшихся клиентов CLIENT_ID_OVERRIDE в config.h, чтобы увести его
// в другой слот.
...
// откуда и коллизии при 2+ клиентах). Слот вычисляется из CLIENT_ID (хэш по
// модулю ACK_SLOT_COUNT), поэтому клиенты с РАЗНЫМИ слотами никогда не
// отвечают одновременно — но два клиента МОГУТ получить один и тот же слот
// (см. разбор вероятности при ACK_SLOT_COUNT=8 ниже), тогда именно эта пара
// будет коллизировать на каждом heartbeat.
//
...
#define ACK_SLOT_COUNT     8       // lora-ds-autonomy: было 16 (запас под будущий рост
                                    // парка), реальный парк — 4 клиента (BOM). См. разбор
                                    // компромисса выше.
